...
def print_memory_usage(num):
    process = psutil.Process(os.getpid())
    mem_info = process.memory_info()
    print(num)
    print(f"mem:{mem_info.rss /1024 /1024:.2f}MB")
    # print(f"内存使用: {mem_info.rss / 1024 / 1024:.2f} MB")  # 以MB为单位
...
class BaseEngine():
    ...
    def train_batch(self):
        ...
        for X, label, his in self._dataloader['train_loader'].get_iterator():
            self._optimizer.zero_grad()

            # X (b, t, n, f), label (b, t, n, 1)
            X, label, his = self._to_device(self._to_tensor([X, label, his]))
            pred = self.model(X, label, his)
    ...
    def evaluate(self, mode):
        ...
        labels = []
        with torch.no_grad():
            for X, label, his in self._dataloader[mode + '_loader'].get_iterator():
                # X (b, t, n, f), label (b, t, n, 1)
                X, label, his = self._to_device(self._to_tensor([X, label, his]))
                pred = self.model(X, label, his)
                pred, label = self._inverse_transform([pred, label])
