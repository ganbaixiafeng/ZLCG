...
class Graph_FusionMLP(nn.Module):
    def __init__(self, input_dimz, hidden_dim, out_dim, graph_num, first, **model_args):
        super().__init__()
        self.input_dimz = input_dimz
        ...
        self.fusion_graph_model = nn.Sequential(
            *[MultiLayerPerceptron(input_dimz=self.input_dimz,
        ...
        self.fusion_model = nn.Sequential(
            *[MultiLayerPerceptron(input_dimz=self.fusion_dim,
    ...
    def forward(self, history_data,
        ...
        if not self.first:
            if self.if_feedback:
                if self.if_forward:
                    node_emb = node_emb[0]
                    hidden_emb = hidden_emb[0]
                    hidden_emb = \
                        self.forward_att(hidden_emb, hidden_emb, hidden_emb)[0]
                    hidden_emb = self.forward_fc(hidden_emb)
                    node_emb = [node_emb * hidden_emb]
...
class MultiLayerPerceptron(nn.Module):
    def __init__(self, input_dimz, hidden_dim, dropout=0.2) -> None:
        super().__init__()

        self.fc = nn.Sequential(
            nn.Linear(in_features=input_dimz, out_features=hidden_dim, bias=True),
            nn.ReLU(),
            nn.Dropout(dropout),
