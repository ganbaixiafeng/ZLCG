...
from ..data.transform import SCALER_REGISTRY
from ..utils.serialization import load_pkl
from ..losses.losses import masked_mae, masked_mape, masked_rmse, masked_mse
from ..metrics.wape import masked_wape
...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: dict):
        ...
        self.metrics = cfg.get("METRICS",
                               {"MAE": masked_mae, "RMSE": masked_rmse, "MAPE": masked_mape, "WAPE": masked_wape,
                                "MSE": masked_mse})
        ...
        # evaluation
        self.if_evaluate_on_gpu = cfg.get("EVAL", EasyDict()).get("USE_GPU",
    ...
    def test(self):
        ...
        for _, data in enumerate(self.test_data_loader):
            ...
            if self.if_evaluate_on_gpu:
                forward_return[0], forward_return[1] = forward_return[0].detach().cpu(), forward_return[
                    1].detach().cpu()
